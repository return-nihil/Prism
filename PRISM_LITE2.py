...
class BandConditioningAware(nn.Module):
    ...
    def __init__(self, n_bands=8, cond_dim=64, latent_dim=8, band_emb_dim=8, heads=2):
        ...
        self.proj_in = nn.Linear(latent_dim + band_emb_dim, cond_dim)
...
class ResidualBlockTuned(nn.Module):
...
class PrismLite(nn.Module):
    def __init__(self,
        ...
        for i in range(n_blocks):
            ...
            block = ResidualBlockTuned(in_channels=channels,
        ...
        self.film = FiLMCondSmall(cond_dim=cond_dim, channels_per_block=channel_schedule, shared=96)
        self.band_cond = BandConditioning(n_bands=n_bands, cond_dim=cond_dim, latent_dim=8, band_emb_dim=8, hidden=128)
    ...
    def forward(self, x, c, state=None):
        ...
        #end = time.time()
        #print(f"PrismLite forward: {end - start:.4f} sec for {T_block} time steps")
...
def print_model_param_summary(model):
    ...
    print("="*60)
    print(f"[InputStem]        : {count_params(model.input_stem):,}")
    print(f"[BandConditioning] : {count_params(model.band_cond):,}")
    print(f"[FiLMCondSmall]    : {count_params(model.film):,}")
