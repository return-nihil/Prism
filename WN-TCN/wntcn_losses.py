...
if __name__ == "__main__":
    batch_size = 2
    signal_length = 2048
    pred = torch.randn(batch_size, signal_length)
    target = torch.randn(batch_size, signal_length)

    criterion = WNTCN_Loss(recon_weigth=1.0, stft_weight=1.0)
    stft_loss, esr_loss = criterion(pred, target)

    print(f"STFT Loss: {stft_loss.item():.6f}, ESR Loss: {esr_loss.item():.6f}")
